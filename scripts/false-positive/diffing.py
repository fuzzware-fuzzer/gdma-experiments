...
class SimpleDMABuf:
    ...
    def __str__(self):
        ...
        alt_str = ""
        if len(self.alt_addrs)>0:
            alt_str += "\n"
            for alt_addr in self.alt_addrs:
                alt_str += f"{alt_addr:#08x}, "
        return f"SimpleDMABuf[{self.mmio_addr:#08x}]: " + size_str + alt_str
...
def extract_buf_definitions_from_groundtruth(groundtruth_yaml_path: Path) -> Dict[Path, List[SimpleDMABuf]]:
...
def scan_for_groundtruth_yamls(groundtruth_root: Path) -> Dict[Path, List[SimpleDMABuf]]:
    res = {}
    ...
    for parent, dirs, files in os.walk(groundtruth_root, topdown=True):
        ...
        res.update(extract_buf_definitions_from_groundtruth(groundtruth_path))
...
def extract_buf_definitions_from_config(model: dict) -> List[SimpleDMABuf]:
    ...
    if descriptors:
        ...
        for descr in descriptors:
            ...
            alt_addrs = []
            # some have alt_addrs, but not all
            try:
                for entry in descr['alt_addrs']:
                    alt_addrs.append(parse_address_value(symbols, entry))
            ...
            res.append(SimpleDMABuf(mmio_addr, buf_sizes, alt_addrs))
...
def generate_diffs(models: List[SimpleDMABuf], other_models: List[SimpleDMABuf]) -> List[DMAModelDiff]:
    ...
    for buf in models:
        ...
        if not other_buf:
            ...
            if not other_bufs_for_same_ram_addr:
                ...
                for other_buf in other_bufs_for_same_ram_addr:
                    # is the identified mmio addr known as an alt addr?
                    if buf.mmio_addr in other_buf.alt_addrs:
                        continue
        ...
        for other_buf in other_models:
            ...
            if is_mmio_addr_missing and not is_already_marked_as_different_mmio_reg and buf.mmio_addr not in other_buf.alt_addrs:
