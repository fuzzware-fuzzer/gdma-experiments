...
def add_fuzzware_commands(strategy, out_path, in_path, strategy_config, runtime, use_python_modeling, stats_only):
    ...
    if not stats_only:
        ...
        command += command_run
    # exp 4 is a case study, so exact block count does not matter
    command_stats = f"sleep {round(random()*MAX_SLEEP_SEC,4)} && fuzzware genstats -p {out_proj_path} --i-am-aware-i-am-overcounting-translation-blocks-so-force-skip-valid-bb-file"
