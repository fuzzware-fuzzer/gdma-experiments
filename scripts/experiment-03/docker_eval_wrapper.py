...
def parse_config(config_name=".config.yml"):
    res = None
    with open(f"{DIR}/03-coverage-experiments/{config_name}", 'r') as f:
...
def main(args):
    config = parse_config()
    command = [ "python3",f"scripts/experiment-03/run_eval_03.py"]
    if args.dry_run:
        command.append("-d")
    if args.stats_only:
        command.append("-s")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
                prog='Run-03',
                description='Docker wrapper to run experiment 03')
