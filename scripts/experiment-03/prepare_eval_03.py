    ...
    for d, dirs, files in os.walk(in_path):
        for file in files:
            if ".elf" in file:
                ...
                relpath = os.path.join("targets", relpath)
...
def copy_to_new_location(target_list, update):
    for src, dst in target_list:
        ...
        # case 2: it exists
            ...
            # print("Target exists, checking hash...")
...
if __name__ == "__main__":
    ...
    parser.add_argument('-i', '--in-path', required=False, type=Path, default=f"{ROOT}/src/03-coverage-experiments/", help='Path that contains sources to build')
    parser.add_argument('-o', '--out-path', required=False, type=Path, default=f"{ROOT}/03-coverage-experiments/", help='Path to move new elfs to')
